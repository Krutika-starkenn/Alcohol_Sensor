...
u8 CAN_Transmit(CanTxMsg* TxMessage)
{
  ...
  /* Check the parameters */
  assert_param(IS_CAN_STDID(TxMessage->g_ul_Can_Tx_STD_ID));
  assert_param(IS_CAN_EXTID(TxMessage->g_ul_Can_Tx_EXD_ID));
  assert_param(IS_CAN_IDTYPE(TxMessage->g_uc_Can_Tx_IDE));
  assert_param(IS_CAN_RTR(TxMessage->g_uc_Can_Tx_RTR));
  assert_param(IS_CAN_DLC(TxMessage->g_uc_Can_Tx_DLC));
  ...
  if (TransmitMailbox != CAN_NO_MB)
  {
    ...
    if (TxMessage->g_uc_Can_Tx_IDE == CAN_ID_STD)
    {
      TxMessage->g_ul_Can_Tx_STD_ID &= (u32)0x000007FF;
      TxMessage->g_ul_Can_Tx_STD_ID = TxMessage->g_ul_Can_Tx_STD_ID << 21;
      
      CAN->sTxMailBox[TransmitMailbox].TIR |= (TxMessage->g_ul_Can_Tx_STD_ID | TxMessage->g_uc_Can_Tx_IDE |
                                               TxMessage->g_uc_Can_Tx_RTR);
    }
    else
    {
      TxMessage->g_ul_Can_Tx_EXD_ID &= (u32)0x1FFFFFFF;
      TxMessage->g_ul_Can_Tx_EXD_ID <<= 3;

      CAN->sTxMailBox[TransmitMailbox].TIR |= (TxMessage->g_ul_Can_Tx_EXD_ID | TxMessage->g_uc_Can_Tx_IDE | 
                                               TxMessage->g_uc_Can_Tx_RTR);
    }
    
    /* Set up the DLC */
    TxMessage->g_uc_Can_Tx_DLC &= (u8)0x0000000F;
    CAN->sTxMailBox[TransmitMailbox].TDTR &= (u32)0xFFFFFFF0;
    CAN->sTxMailBox[TransmitMailbox].TDTR |= TxMessage->g_uc_Can_Tx_DLC;

    /* Set up the data field */
    CAN->sTxMailBox[TransmitMailbox].TDLR = (((u32)TxMessage->g_uc_Can_Tx_DATA[3] << 24) | 
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[2] << 16) |
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[1] << 8) | 
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[0]));
    CAN->sTxMailBox[TransmitMailbox].TDHR = (((u32)TxMessage->g_uc_Can_Tx_DATA[7] << 24) | 
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[6] << 16) |
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[5] << 8) |
                                             ((u32)TxMessage->g_uc_Can_Tx_DATA[4]));
...
* Return         : None.
*******************************************************************************/
void get_Received_CAN_Data(u8 FIFONumber, CanRxMsg* RxMessage)
{
  /* Check the parameters */
  assert_param(IS_CAN_FIFO(FIFONumber));

  /* Get the Id */
  RxMessage->g_uc_Can_Rx_IDE = (u8)0x04 & CAN->sFIFOMailBox[FIFONumber].RIR;
  if (RxMessage->g_uc_Can_Rx_IDE == CAN_ID_STD)
  {
    RxMessage->g_ul_Can_Rx_STD_ID = (u32)0x000007FF & (CAN->sFIFOMailBox[FIFONumber].RIR >> 21);
  }
  else
  {
    RxMessage->g_ul_Can_Rx_EXD_ID = (u32)0x1FFFFFFF & (CAN->sFIFOMailBox[FIFONumber].RIR >> 3);
  }
  
  RxMessage->g_uc_Can_Rx_RTR = (u8)0x02 & CAN->sFIFOMailBox[FIFONumber].RIR;

  /* Get the DLC */
  RxMessage->g_uc_Can_Rx_DLC = (u8)0x0F & CAN->sFIFOMailBox[FIFONumber].RDTR;

  /* Get the FMI */
  RxMessage->g_uc_Can_Rx_FMI = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDTR >> 8);

  /* Get the data field */
  RxMessage->g_uc_Can_Rx_DATA[0] = (u8)0xFF & CAN->sFIFOMailBox[FIFONumber].RDLR;
  RxMessage->g_uc_Can_Rx_DATA[1] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDLR >> 8);
  RxMessage->g_uc_Can_Rx_DATA[2] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDLR >> 16);
  RxMessage->g_uc_Can_Rx_DATA[3] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDLR >> 24);

  RxMessage->g_uc_Can_Rx_DATA[4] = (u8)0xFF & CAN->sFIFOMailBox[FIFONumber].RDHR;
  RxMessage->g_uc_Can_Rx_DATA[5] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDHR >> 8);
  RxMessage->g_uc_Can_Rx_DATA[6] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDHR >> 16);
  RxMessage->g_uc_Can_Rx_DATA[7] = (u8)0xFF & (CAN->sFIFOMailBox[FIFONumber].RDHR >> 24);
